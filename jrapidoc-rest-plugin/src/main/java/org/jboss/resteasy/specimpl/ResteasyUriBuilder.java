...
public class ResteasyUriBuilder extends UriBuilder {
    ...
    /**
     * Only replace path params in path of URI. This changes state of
     * URIBuilder.
     * 
     * @param name
     * @param value
    ...
    protected StringBuffer replaceParameter(Map<String, ? extends Object> paramMap, boolean fromEncodedMap, boolean isTemplate, String string,
    ...
    protected StringBuffer replaceQueryStringParameter(Map<String, ? extends Object> paramMap, boolean fromEncodedMap, boolean isTemplate,
     ...
     * encode {@code %} characters that are followed by two hex characters). The
     * JavaDoc for {@code @QueryParam.value()} explicitly states that the value
     * is specified in decoded format and that "any percent encoded literals
     * within the value will not be decoded and will instead be treated as
     * literal text". This means that it is an explicit bug to perform
     * contextual URI encoding of this method's name parameter; hence, we must
     * always encode said parameter. This method also foregoes contextual URI
     * encoding on this method's value parameter because it represents arbitrary
     * data passed to a {@code QueryParam} parameter of a client proxy (since
     * the client proxy is nothing more than a transport layer, it should not be
     * "interpreting" such data; instead, it should faithfully transmit this
     * data over the wire).
     * </ul>
     * 
     * @param name
